...
| 모드 | 도구 | 할 수 있는 것 | 기본 |
|---|---|---|---|
| **A. 읽기 전용** (`readonly`) | `Read` + `Bash` | 어디든 읽고 탐색하되 아무것도 바꾸지 못한다 | — |
| **B. 편집** (`edit`) | `+ Write` + `Edit` | **작업 디렉토리 안의 파일을 만들고 고친다.** 그 밖은 여전히 읽기 전용 | **기본값** |

두 모드가 **공유하는 것**: 작업 디렉토리 밖은 양쪽 다 읽기 전용이고, **삭제·이동·시스템 변경·
패키지 설치·git 히스토리 조작은 양쪽 다 거부**된다. "작업 중인 파일을 고쳐도 된다" 와 "rm 을
실행해도 된다" 는 다른 요구이고, 요청된 것은 앞의 것뿐이다.

edit 모드는 도구 allowlist 를 **정확히 두 개** 넓힐 뿐 가드를 끄지 않는다 — 그 밖의 도구가 붙으면
패널은 여전히 전송을 거부한다 (회귀 32 가 두 모드 모두 고정).
...
부수 효과로 토큰 소비도 컸다 — 같은 2턴이 $0.387 → $0.101 상당. 같은 답에 토큰 4배다.

### `/tmp` 구멍 — edit 모드를 켜자 드러났다 (실측)

edit 모드 첫 실행에서 **작업 디렉토리 밖 쓰기가 성공했다.** 어시스턴트가 스스로 "경계가 적용되지
않고 있다" 고 보고했다.

원인은 런처의 `--bind /tmp /tmp` 였다. 호스트 `/tmp` 전체가 쓰기 가능했고, **readonly 모드에서도
마찬가지였다** — 그동안 2계층(plan mode)이 가려주고 있었을 뿐 1계층에는 처음부터 구멍이 있었다.

`--tmpfs /tmp` 로 바꿨다. CLI 는 자기 스크래치 공간을 얻고 호스트 `/tmp` 에는 접근하지 못한다.
재검증 결과 같은 쓰기가 차단된다.

교훈: **쓰기를 허용해 보기 전까지 이 구멍은 보이지 않았다.** 읽기 전용으로만 시험하면 1계층의
결함이 2계층에 가려진다. 그래서 README 의 확인 절차에 edit 모드 항목을 넣었다.
...
- **세션을 질문마다 새로 만들지 않는다.** 세션 시작이 비싼 이벤트다 — 실측으로 1턴은 cache
  creation 12,433 + cache read 8,568 토큰이고, 2턴은 cache read 21,001 로 거의 전부 캐시에서
  온다 ($0.0832 → 증분 $0.0173, 약 1/5). `ClaudeCliBackend` 가 프로세스를 열어두고 stdin 으로
  턴을 넘기는 이유가 이것이다. 질문마다 프로세스를 띄우면 매번 세션 시작을 낸다.
  덧붙여 `system-prompt.md` 를 줄이는 것은 지렛대가 아니다 — 그 ~21k 접두어 중 ~1,500 토큰일
  뿐이고(사용자 `CLAUDE.md` 가 ~2,300, 나머지는 CLI 자체 프롬프트와 도구 정의), 숫자를 움직이는
  것은 세션 수와 도구 개수다.
...
| 5 | Mode B (쓰기 허용) — 모드 스위치, 작업 디렉토리만 쓰기 가능, 삭제·시스템은 계속 차단 | **완료** |
...
- ~~stream-json 이벤트 스키마 미검증~~ → **해결.** 실물 세션에서 잡았고 회귀 32 가 고정한다.
  ...
  실패로 올린다).
- ~~비용 표시 미검증~~ → **해결.** `result.total_cost_usd` 로 누계가 온다. 단 **그것이 돈인지는
  인증 방식에 달렸고 패널은 알 수 없다** — 이 머신은 `apiKeySource: "none"`(구독 로그인)이고
  `rateLimitType: "five_hour"` / `overageStatus: "rejected"` 라 질문당 청구가 아니라 사용량 창을
  깎으며, 초과하면 거부된다. `ANTHROPIC_API_KEY` 가 설정된 사용자에게는 실제 과금이다. 그래서
  상태줄 문구는 "spent" 가 아니라 "token usage ... equivalent" 로 쓴다.
