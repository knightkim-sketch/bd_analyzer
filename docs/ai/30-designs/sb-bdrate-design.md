...
status: implemented (1-3단계). 4-5단계 미착수
created: 2026-09-08
updated: 2026-09-08
author: claude-opus-5
verified: "단위 테스트 45항목 + 회귀 31. 실제 X 화면 렌더링은 미검증"
...
| 4 | Sequence 패널 — GUI 스레드 슬라이스 + 진행률 + 취소 (B-0) | 전 구간 |
...
- **`bits == 0` 인 SB 의 처리 방침**이 미결이다. 로그를 취할 수 없어 점을 버리는데, skip 된
  superblock 은 "0 bits 로 그 품질을 얻었다" 는 의미 있는 정보다. `bits+1` 로 대체하는
  관행이 있으나 값을 만드는 것이라 임의로 넣지 않았다.
