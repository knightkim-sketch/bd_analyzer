...
디코딩된 YUV 와 블록 통계는 **실행한 디렉토리**의 `.bd_analyzer/` 에 쌓인다
(`BDCachePaths`). 바이너리 위치가 아니라 `cd` 해서 들어간 위치다.

```bash
cd /data/work          # 용량이 있는 볼륨
bd-analyzer a.ivf      # -> /data/work/.bd_analyzer/
```

디코딩 YUV 는 GB 단위로 늘어난다. `/opt` 설치본이 실행 파일 옆에 쓸 수 없어
`~/.cache/` 로 폴백하던 방식은 홈 파티션이 작은 머신에서 문제가 됐고, 위치를
사용자가 고를 수 없었다. 그래서 실행 위치를 따라간다.

- 쓰기 불가한 디렉토리에서 실행하면 **캐시 없이** 동작한다 (다른 곳으로 폴백하지 않는다).
- 위치를 고정하려면 설정 `BDCache/directory` 를 쓴다.
- 종료 시 캐시는 묻지 않고 삭제된다 — 원본에서 다시 만들 수 있는 파일이다.
