...
**열 수 없는 파일이 playlist 에 들어 있으면 앱이 시작할 때마다 SIGSEGV 로 죽었습니다.**

playlist 항목을 복원하는 경로가 "파일은 당연히 열린다"고 가정하고 있었습니다. 압축 스트림 항목의
생성자는 열기에 실패하면 — libavcodec 이 거부하거나, raw 포맷을 모르거나, 프레임 크기나 픽셀
포맷을 못 얻으면 — 영상 핸들러를 만들기 전에 돌아 나옵니다. 복원 코드는 그 핸들러를 검사 없이
역참조했습니다.

여는 데 실패하는 것 자체는 원래 "이유를 붙여 표시하고 넘어가는" 동작인데, **복원 경로에서만
치명적이었습니다.** 그 항목이 자동저장 playlist 에 남아 있으면 탈출구가 없어서, 설정을 손으로
지우기 전까지 앱을 띄울 수 없었습니다.

- 스펙을 위반한 AV1 스트림이 정확히 이런 파일입니다 — 이 도구가 들여다보라고 있는 종류의
  스트림입니다.
- 이제 복원되지 않는 항목은 playlist 에서 빠지고, 앱은 정상적으로 뜹니다.

> **0.3.1 이하에서 이 증상을 만나셨다면** 아래로 자동저장 playlist 만 비우시면 즉시 복구됩니다
> (다른 설정은 유지됩니다).
>
> ```
> CONF="$HOME/.config/Institut für Nachrichtentechnik, RWTH Aachen University/YUView.conf"
> cp -p "$CONF" "$CONF.bak"
> sed -i '/^Autosaveplaylist=/d' "$CONF"
> ```
